module Pelt
  module Frameworks
    ...
    ALL = []
    DEFAULT_FRAMEWORKS_PATH = File.join(Pelt.base_directory, 'frameworks')

    class Framework
      attr_accessor :name, :path, :templates_directory, :stylesheets_directory
      def initialize(name, *arguments)
        options = arguments.last.is_a?(Hash) ? arguments.pop : {}
        self.path = path = options[:path] || arguments.shift
        @name = name
        @template_markup_directories = options[:templates_directory] || File.join(path, 'templates')
      ...
      def template_markup_directories
          Dir.entries(templates_directory).map{|d|File.directory?(d) && !d.match(/^\./) ? d : nil }.compact
      end
      def template_directories
        Dir.glob(File.join(template_markup_directories||templates_directory, "*")).map{|f| File.basename(f)}
      end
      ...
      def manifest(pattern, options = {})
        ...
        Pelt::Installers::Manifest.new(manifest_file(pattern), options)
      end
    end
    ...
    def register_directory(directory)
      loaders = [
        File.join(directory, "pelt_init.rb"),
...
Pelt::Frameworks.discover(:defaults)
